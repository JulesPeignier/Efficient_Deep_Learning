...
class BC():
    ...
    def binarization(self):
        ...
        for weight_index in range(len(self.target_modules)):
            #print(self.target_modules[weight_index].data)

            self.target_modules[weight_index].data.copy_(torch.sign(self.target_modules[weight_index].data)) # = m.weight.data
            # torch.sign() sets positive values to 1 and negative values to -1.
    ...
    def clip(self):
        ...
        for index in range(self.num_of_params):
            self.saved_params[index].copy_(hardtanh(self.target_modules[index].data)) # formule
